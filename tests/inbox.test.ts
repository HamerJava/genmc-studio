...
import {enqueueMessage,markMessagesRead,UserActionChannel,type AgentMessage} from '../lib/skin/inbox';
...
test('receipts acknowledge only delivered messages and retain newer queued text',()=>{
 ...
 const room=enqueueMessage(markMessagesRead(allQueued,['3']),message('17'));assert.equal(room.length,16);assert.ok(!room.some(m=>m.id==='3'));assert.equal(room.at(-1)?.readAt,null);
});

...
export async function body(req:Request, limit=100000){if(Number(req.headers.get('content-length')||0)>limit)throw Error('Request too large');const t=await req.text();if(t.length>limit)throw Error('Request too large');return JSON.parse(t)}

...
export function enqueueMessage(
  ...
  const next = [...messages];
  if (next.length >= 16) {
    const index = next.findIndex((m) => m.readAt !== null);
    if (index < 0)
      throw Error(
        'Your queue is full. Let your agent read the waiting messages first.',
      );
    next.splice(index, 1);
  }
  return [...next, message];
...
export class UserActionChannel {
  ...
  wait(
    ...
    if (!Number.isInteger(timeoutMs) || timeoutMs < 0 || timeoutMs > 15000)
      throw Error('timeoutMs must be between 0 and 15000.');

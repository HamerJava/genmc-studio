...
## Split view and visibility update (2026-09-04)

Thirteen automated tests, TypeScript and the production build pass. The new color test checks exact RGB matches across base and overlay, excludes transparent/unused pixels, and verifies that highlighting does not mutate the texture. Native local WebMCP selected a color and updated both previews to the same six-pixel match count. A one-pixel tool edit changed both counts to one; undo restored the original texture and zero matches. Body-map click, Shift-click isolation and Show all were checked against native skin state. The inspector contains no dropdowns. Desktop and 390×844 mobile previews were inspected; mobile Split stacks the previews, the atlas remains square, and document width equals viewport width. The existing video predates this UI.
